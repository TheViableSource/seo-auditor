...
export default function DashboardPage() {
    ...
    return (
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
            ...
            {sites.filter(s => s.auditSchedule && s.auditSchedule !== "manual").length > 0 && (
                <Card className="shadow-sm border-zinc-200">
                    <CardHeader className="pb-3 flex flex-row items-center justify-between">
                        <CardTitle className="text-base flex items-center gap-2">
                        ...
                        <Link href="/settings" className="text-xs text-orange-500 hover:underline flex items-center gap-0.5">
                    ...
                    <CardContent className="p-0 px-4 pb-4">
                        <div className="space-y-3">
                            {sites
                                .filter(s => s.auditSchedule && s.auditSchedule !== "manual")
                                .map(site => {
                                    const latest = getLatestAuditForSite(site.id)
                                    ...
                                    return (
                                        <div key={site.id} className="flex items-center justify-between gap-3 py-2 border-b border-border/50 last:border-0">
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                <div className="w-7 h-7 rounded-md bg-violet-50 dark:bg-violet-950/30 flex items-center justify-center">
                                                    <Globe className="h-3.5 w-3.5 text-violet-500" />
                                                </div>
                                                <div className="min-w-0">
                                                    <p className="text-sm font-medium truncate">{site.name || site.domain}</p>
                                                    <p className="text-[10px] text-muted-foreground">
                                                        {schedLabel[site.auditSchedule || ""] || site.auditSchedule}
                                                        {site.lastScheduledAudit && ` · Last: ${new Date(site.lastScheduledAudit).toLocaleDateString()}`}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {latest ? (
                                                    <span className={`text-sm font-bold ${latest.score >= 80 ? "text-green-600" : latest.score >= 60 ? "text-orange-600" : "text-red-600"}`}>
                                                        {latest.score}
                                                    </span>
                                                ) : (
                                                ...
                                                {latest && (() => {
                                                    const prev = (() => {
                                                        try {
                                                            const audits = getRecentAudits(50).filter(
                                                                a => a.domain === site.domain
                                                            )
                                                            return audits.length > 1 ? audits[1] : null
                                                        } catch { return null }
                                                    })()
                                                    if (!prev) return null
                                                    const delta = latest.score - prev.score
                                                    if (delta === 0) return <Minus className="w-3 h-3 text-muted-foreground" />
                                                    return delta > 0
