...
import { mockSites, type MockSite } from "@/lib/mock-data"
import Link from "next/link"

function ScoreMeter({ score }: { score: number }) {
    const getColor = (s: number) => {
        if (s >= 80) return { ring: "text-green-500", bg: "bg-green-50", text: "text-green-700" }
        if (s >= 60) return { ring: "text-orange-500", bg: "bg-orange-50", text: "text-orange-700" }
...
function StatusIndicator({ status }: { status: MockSite["status"] }) {
    const config = {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-card rounded-xl shadow-xl border border-border w-full max-w-md p-6 space-y-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex gap-3 justify-end">
                    ...
                    <Button className="bg-orange-500 hover:bg-orange-600 text-white gap-2" onClick={onClose}>
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
            ...
            {filteredSites.length === 0 && (
                <div className="text-center py-16">
                    ...
                    <p className="text-muted-foreground mt-1">
                        {searchQuery ? "Try a different search term." : "Add your first website to get started."}
                    </p>
                </div>
            )}

            {/* Add Site Dialog */}
            <AddSiteDialog open={showAddDialog} onClose={() => setShowAddDialog(false)} />
        </div>
    )
}
